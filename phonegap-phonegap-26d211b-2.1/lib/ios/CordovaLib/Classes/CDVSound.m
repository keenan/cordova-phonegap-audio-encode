...
//:SL
- (void) getCurrentAudioRecLevel:(CDVInvokedUrlCommand*)command
{
	NSString* callbackId = command.callbackId;
	NSString* mediaId = [command.arguments objectAtIndex:0];
#pragma unused(mediaId)
    ...
    if (audioFile != nil && audioFile.recorder != nil && [audioFile.recorder isRecording]){
		if(!audioFile.recorder.isMeteringEnabled)
			audioFile.recorder.meteringEnabled = true; // Enable metering on first use
		[audioFile.recorder updateMeters];
		level = [audioFile.recorder averagePowerForChannel:0];
		//NSLog(@"AudioRec Level %f", level);
    }
    CDVPluginResult* result = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDouble: level];
	NSString* jsString = [NSString stringWithFormat: @"%@(\"%@\",%d,%.3f);\n%@", @"cordova.require('cordova/plugin/Media').onStatus", mediaId, MEDIA_RECLEVEL, level, [result toSuccessCallbackString:callbackId]];
	//NSString* jsString = [NSString stringWithFormat: @"%@", [result toSuccessCallbackString:callbackId]];
    [super writeJavascript:jsString];
...
- (void) stopRecordingAudio:(CDVInvokedUrlCommand*)command
{
	...
	if (audioFile != nil && audioFile.recorder != nil) {
		...
		audioFile.recorder.meteringEnabled = false; // Disable level metering
		[audioFile.recorder stop];
